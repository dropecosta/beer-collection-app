  ...
  return (
    <Link href={`/beer/${beer.id}`} className="block">
      <Card>
        ...
        <CardFooter>
          <span className="text-xs text-gray-500">{`ABV: ${beer.abv ?? '–'}%`}</span>
          <span className="text-xs text-gray-500">{`IBU: ${beer.ibu ?? '–'}`}</span>
          <span className="text-xs text-gray-500">{`EBC: ${beer.ebc ?? '–'}`}</span>
        </CardFooter>
      </Card>
    </Link>
  )
